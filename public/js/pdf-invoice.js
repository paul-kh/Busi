...
console.log("invoiceID from Session: ", invoiceID);
...
axios.get(`/api/invoices/${invoiceID}`)
  .then(invoice => {
    console.log(invoice);
    console.log("order id: ", invoice.data[0].salesorder_id);
    // INVOICE FOOTER
    ...
    // .... get order info
    axios.get(`api/salesorders/${invoice.data[0].salesorder_id}`).then(order => {
